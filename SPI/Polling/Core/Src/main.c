...
static HAL_StatusTypeDef MPU6500_WriteReg(uint8_t reg, uint8_t data)
{
    ...
    tx[0] = reg & 0x7F;   // SPI write: bit7 = 0
    tx[1] = data;
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
	...
	/* 2. Wake up MPU6500: PWR_MGMT_1 = 0x00 */
	writeData = 0x00;

	ret = MPU6500_WriteReg(PWR_MGMT_1, writeData);
	...
	UART_Print("Start reading ACCEL data...\r\n");
  /* USER CODE END 2 */
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
		ret = MPU6500_ReadBytes(ACCEL_XOUT_H, accelData, 6);
		...
		{
				UART_Print("Read ACCEL data failed.\r\n");
		}
